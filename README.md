...
- The `--list` switch may be used to identify .properties/.tar.gz/.apk.gz file sets for installing back on the phone or manual deletion.  Note that `--list` skips the backup set to archive set comparison and archival transactions, and only list the current content of the archive.
